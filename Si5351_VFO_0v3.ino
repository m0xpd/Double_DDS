...
Now with OnLine IF Alignment (see Menu 4)
...
*/
double BFOs[]={
  9999550, 9997400};  // set these for YOUR IF !
...
// Display...
// the display & Si5313 use the I2C connection, 
...
const int nMenus = 0x04;
byte nMenuOptions[] = {
  1, 1, 1, 5, 1};
char* MenuString[5][12]= {
  {
    "RiT:  ", "      " }
  ,
  {
...
void loop() {
  ...
  if (MenuMode){
    ...
    // New "OnLine Alignment" Feature - Feb 2015...
    if (Menu==4){
    if (Turns!=0){
    BFOs[MenuOption[2]]=constrain(BFOs[MenuOption[2]]+Turns*50,1000000,15000000);
    Turns=0;
    BFO=BFOs[MenuOption[2]];
    sendbfo(BFO);
    sendFrequency(freq);  // update the VFO too 4 Feb 2015
    LCD_Display_BFO(BFO);        
    }  
    }     
...
void LCD_Display_Freq2(double frequency) {  
